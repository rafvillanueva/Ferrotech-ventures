...
<head>
     ...
     <title>Projectbased Employee</title>
...
<div class="pull-left1">
  <h2 style="font-weight:bold; font-family: 'Corbel'; color:rgb(150, 8, 10); letter-spacing: 2px;"> 
    <span><img src="images/casuallist.jpg" height="40px"></span> Project Based Employee List
  </h2>
</div>
<hr>
      ...
      if(isset($_POST['search_rage'])){
          ...
          if(empty($val)){
            ...
            if(empty($from) || empty($to)){
              $dd = "Department: " . $dep;
              $link = "projectbasedeval_print/?department=" . $dep;
            }elseif($dep == "-- DEPARTMENT --"){
              $dd = "From " . $from . " - To: " . $to;
              $link = "projectbasedeval_print/?zfrom=" . $l_from . "&zto=" . $l_to;
            }else{
              $dd = "From " . $from . " - To: " . $to;
              $link = "projectbasedeval_print/?from=" . $l_from . "&to=" . $l_to . "&zdepartment=" . $dep;
            }
          }else{
            $dd = "Result";
            $link = "projectbasedeval_print/?value=" . $val;
          }
...
<div class="panel panel-default">
  <div class="panel-heading">
    Reports of Project Based Employee.
    ...
    <table class="table table-hover" >
      ...
      <tbody>
        <?php
          ...
          $val = $_POST['search_v'];
          $query = mysql_query("SELECT * from tbl_applicants WHERE a_status = 'Project Based' AND (a_id LIKE '%$val%' OR a_lname LIKE '%$val%' OR a_fname LIKE '%$val%' OR a_mname LIKE '%$val%') AND a_status = 'Project Based'")or die(mysql_error());
        }elseif(isset($_POST['search_valuated'])){
          $query = mysql_query("SELECT * from tbl_ratings WHERE a_type = 'Project Based' AND a_rate NOT LIKE ''")or die(mysql_error());
        }elseif(isset($_POST['search_not_valuated'])){
          ...
          while($rowz=mysql_fetch_array($query)){
            $id = $rowz['a_id'];
            $info = mysql_query("SELECT * from tbl_applicants where a_id = '$id' AND `tbl_applicants`.a_status = 'Project Based' ORDER BY ID DESC")or die(mysql_error());
            ...
            if(!empty($row['a_lname'])){
              if(isset($_POST['search_not_valuated'])){
                $cc = mysql_query("SELECT * from tbl_ratings WHERE a_type = 'Project Based' AND a_id = '$id'")or die(mysql_error());
