...
<th colspan="8" style="font-weight: bold; background-image: url(images/r.jpg); background-repeat: no-repeat; background-size: cover; color:white; font-size:20px; "> <img src="images/waiting.png" height="50px" width="50px"><b style="letter-spacing: 2px; font-size: 20px; letter-spacing:2px;"> Applicant List</b>
...
<button type="button"  class="btn btn-success" data-toggle="modal" data-target="#addapp" >
<span class="glyphicon glyphicon-plus-sign"></span>&nbsp Add Applicant </button>
  ...
  <tbody>
    <?php
      ...
      if(isset($_POST['search_val'])){
        $val = $_POST['search_v'];
        $query=mysql_query("SELECT * from tbl_applicants where a_status = '' AND (a_fname like '%$val%' OR a_lname like '%$val%' OR a_mname like '%$val%' OR a_position like '%$val%') ORDER BY ID DESC")or die(mysql_error());
      }else{
        $query=mysql_query("SELECT * from tbl_applicants where a_status = '' ORDER BY ID DESC")or die(mysql_error());
      }
    ...
    <tr style="text-transform: uppercase; font-size: 13px;">
      ...
      <td><center><a href="waiting_list_view.php?id=<?= $row['ID'] ?>" class="btn btn-xs btn-success">View</a></center></td>
    </tr>
    <?php
    $num++;
    }
