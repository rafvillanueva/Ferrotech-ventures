...
<head>
     ...
     <title>Regular Evaluation</title>
...
<th colspan="8" style="font-weight: bold; background-image: url(images/r.jpg); background-repeat: no-repeat; background-size: cover; color:white; font-size:20px; "> <img src="images/emp_rating.png" height="50px" width="50px"><b style="letter-spacing: 2px; font-size: 20px; letter-spacing:2px;"> Regular Employee Ratings</b>
...
<form method="POST"  style="margin-right:9px; margin-top: 10px;">
 ...
 <input type="text" name="search_v" class="search-query form-control" placeholder="Search Employee.." required>
  ...
  <?php
    if(isset($_POST['search_val'])){
      $val = $_POST['search_v'];
      $query = mysql_query("SELECT * from tbl_applicants where a_status = 'Regular' AND (a_lname like '%$val%' OR a_fname like '%$val%' OR a_mname like '%$val%' OR a_id like '%$val%')")or die(mysql_error());
    }else{
      $query = mysql_query("SELECT * from tbl_applicants where a_status = 'Regular'")or die(mysql_error());
    }
    $count_notify = 0;
 ...
 if(mysql_num_rows($query) == 0){
  ...
  <tr style="text-transform: uppercase; font-size: 13px;">
    ...
    if($end <= date("Y-m-d")){
      ?>
      <td><center><a href="regular_endconform.php?id=<?= $row['a_id'] ?>&casual" class="btn btn-xs btn-danger" style="font-size: 10px;">End Con.</a></center></td>
      <?php
    }else{
      $start_date = $end;  
      $date = strtotime($start_date);
      ...
      if($d  <= date("Y-m-d")){        
        ?>
        <td><center><a href="regularevaluation.php?id=<?= $row['a_id'] ?>&projectbased" class="btn btn-xs btn-warning" style="font-size: 10px;">Exp. Soon</a></center></td>
        <?php
      }else{        
        ?>
        <td><center><a href="regularevaluation.php?id=<?= $row['a_id'] ?>&regular" class="btn btn-xs btn-success" style="font-size: 10px;">Evaluate</a></center></td>
        <?php
      }
    }
    ?>
