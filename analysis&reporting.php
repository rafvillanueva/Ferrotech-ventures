...
<div class="pull-left1">
  <h2 style="font-weight:bold; font-family: 'Corbel'; color:rgb(150, 8, 10); letter-spacing:2px;"> 
    <span><img src="images/casuallist.jpg" height="40px"></span> Casual Employee List
  </h2>
</div>
<hr>
  ...
  <div class="pull-rSight">
    <div class="form-group">
     ...
     <select class="form-control" name="department">
      ...
      if(isset($_POST['search_rage'])){
          ...
          if(empty($val)){
            ...
            if(empty($from) || empty($to)){
              $dd = "Department: " . $dep;
              $link = "casualeval_print/?department=" . $dep;
            }elseif($dep == "-- DEPARTMENT --"){
              $dd = "From " . $from . " - To: " . $to;
              $link = "casualeval_print/?zfrom=" . $l_from . "&zto=" . $l_to;
            }else{
              $dd = "From " . $from . " - To: " . $to;
              $link = "casualeval_print/?from=" . $l_from . "&to=" . $l_to . "&zdepartment=" . $dep;
            }
          }else{
            $dd = "Result";
            $link = "casualeval_print/?value=" . $val;
          }
      ...
      ?>
      <a href="<?= $link ?>" target="_blank" class="btn btn-primary" type="submit" name="search_valz" style="font-size: 13px; width: 35%; margin-left:65%; margin-top: -5%;">Print Report ( <b><i><?= $dd ?></i></b> )</a>
      ...
      <tbody>
        <?php
          ...
          if(empty($val)){
            ...
            $query = mysql_query("SELECT * from tbl_applicants WHERE a_status = 'Casual' AND (a_id LIKE '%$val%' OR a_lname LIKE '%$val%' OR a_fname LIKE '%$val%' OR a_mname LIKE '%$val%')")or die(mysql_error());
          }
        }elseif(isset($_POST['search_val'])){        
          $val = $_POST['search_v'];
          $query = mysql_query("SELECT * from tbl_applicants WHERE a_status = 'Casual' AND (a_id LIKE '%$val%' OR a_lname LIKE '%$val%' OR a_fname LIKE '%$val%' OR a_mname LIKE '%$val%') AND a_status = 'Casual'")or die(mysql_error());
        }elseif(isset($_POST['search_valuated'])){
          $query = mysql_query("SELECT * from tbl_ratings WHERE a_type = 'Casual' AND a_rate NOT LIKE ''")or die(mysql_error());
        }elseif(isset($_POST['search_not_valuated'])){
          $query = mysql_query("SELECT * from tbl_applicants where a_status = 'Casual' ORDER BY ID DESC")or die(mysql_error());
        }elseif(isset($_POST['search_all'])){          
          $query = mysql_query("SELECT * from tbl_applicants where a_status = 'Casual' ORDER BY ID DESC")or die(mysql_error());
        }else{
          $query = mysql_query("SELECT * from tbl_applicants where a_status = 'Casual' ORDER BY ID DESC")or die(mysql_error());
          ...
          while($rowz=mysql_fetch_array($query)){
            $id = $rowz['a_id'];
            $info = mysql_query("SELECT * from tbl_applicants where a_id = '$id' AND `tbl_applicants`.a_status = 'Casual' ORDER BY ID DESC")or die(mysql_error());
            ...
            if(!empty($row['a_lname'])){
              if(isset($_POST['search_not_valuated'])){
                $cc = mysql_query("SELECT * from tbl_ratings WHERE a_type = 'Casual' AND a_id = '$id'")or die(mysql_error());
