...
<th colspan="8" style="font-weight: bold; background-image: url(images/r.jpg); background-repeat: no-repeat; background-size: cover; color:white; font-size:20px; "> <img src="images/emp_rating.png" height="50px" width="50px"><b style="letter-spacing: 2px; font-size: 20px; letter-spacing:2px;"> Casual Employee Ratings</b>
...
<form method="POST"  style="margin-right:9px; margin-top: 10px;">
 ...
 if(mysql_num_rows($query) == 0){
  ...
  <tr style="text-transform: uppercase; font-size: 13px;">
    ...
    if($end <= date("Y-m-d")){
      ?>
      <td><center><a href="casual_endconform.php?id=<?= $row['a_id'] ?>&casual" class="btn btn-xs btn-danger" style="font-size: 10px;">End Con.</a></center></td>
      <?php
    }else{
      $start_date = $end;  
      $date = strtotime($start_date);
      ...
      if($d  <= date("Y-m-d")){        
        ...
        ?>
        <td><center><a href="casualevaluation.php?id=<?= $row['a_id'] ?>&casual" class="btn btn-xs btn-success" style="font-size: 10px;">Evaluate</a></center></td>
        <?php
      }
    }
    ?>
